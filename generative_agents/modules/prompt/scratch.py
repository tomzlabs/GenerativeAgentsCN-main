...
class Scratch:
    ...
    def prompt_schedule_daily(self, wake_up, daily_schedule):
        ...
        def _callback(response):
            # response is parsed as List[ScheduleEntry]
            entries = response  # magentic unwraps the single field 'res' if possible? 
            # Wait, magentic unwraps if the return type of the decorated function is not the Model itself?
            # In llm_model.py: output = response(_prompt).res
            # So output IS the list of ScheduleEntries.
            
            # Let's verify what response is passed to _callback.
            # In llm_model.py: if callback: response = callback(output)
            # output = response(_prompt).res
            # So response passed to _callback is List[ScheduleEntry]
            
            data = {}
            for entry in response:
                if isinstance(entry, ScheduleEntry):
                    data[entry.time] = entry.activity
                else:
                    # Fallback if something weird happens (unlikely with pydantic)
                    pass
            
            assert len(data) >= 5, "less than 5 schedules"
            return data
    ...
    def prompt_schedule_decompose(self, plan, schedule):
        ...
        class schedule_decomposeResponse(BaseModel):  
            res: List[DecomposeEntry]

        def _callback(response):
            # Convert back to list of tuples for internal logic
            response_tuples = []
            for entry in response:
                if isinstance(entry, DecomposeEntry):
                    response_tuples.append((entry.describe, entry.duration))
            
            left = plan["duration"] - sum([s[1] for s in response_tuples])
            if left > 0:
                response_tuples.append((plan["describe"], left))
    ...
    def prompt_schedule_revise(self, action, schedule):
        ...
        class schedule_reviseResponse(BaseModel):
            res: List[ReviseEntry]

        def _callback(response):  
            # response已经是List[ReviseEntry]类型  
            decompose = []
            # Convert ReviseEntry to tuple (start, end, describe) for logic
            for entry in response:
                if isinstance(entry, ReviseEntry):
                    start, end, describe = entry.start, entry.end, entry.describe
    ...
    def prompt_describe_event(self, subject, describe, address, emoji=None):
        ...
        def _callback(response):  
            # response已经是List[EventEntry]类型  
            for entry in response:
                if isinstance(entry, EventEntry):
                    subject, predicate, obj = entry.subject, entry.predicate, entry.object
                    # 验证三元组不为空  
    ...
    def prompt_reflect_insights(self, nodes, topk):
        ...
        class reflect_insightsResponse(BaseModel):  
            res: List[InsightEntry]

        def _callback(response):  
            insights = []  
            for entry in response:
                if isinstance(entry, InsightEntry):
                    insight, node_ids_str = entry.insight, entry.node_ids
